...
test('desktop queue keeps native table semantics and a real editor link', async ({ page }) => {
  ...
  const mobileList = page.locator('[data-platform-list][aria-label="블로그 콘텐츠 모바일 목록"]')
  await expect(page.locator('[data-platform-table][aria-label="블로그 콘텐츠 목록"]')).toHaveCount(1)
  await expect(mobileList).toHaveCount(1)
