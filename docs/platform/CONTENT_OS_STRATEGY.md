...
Codex는 글을 작성하기 전에 문장군_브랜드에서 브랜드 사실, 금지표현, 현장 판단 기준을 조사하고, 이 프로젝트의 CMS에서 기존 글, 카테고리, 중복 주제, 발행 상태를 조사한다. 이 조사는 관리자 화면의 AI 생성 server action이 아니라 외부 작성 업무에서 수행한다. 별도 블로그 운영 저장소의 주제 큐·원고·발행 URL은 이 CMS와 연결하거나 동기화하지 않는다.
