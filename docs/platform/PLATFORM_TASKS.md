...
현재 통합 기준점은 `v2-cms`를 base로 한 `codex/platform-admin-blog-stabilization`이며 통합 Draft PR은 [#73](https://github.com/westgeneraldoor/munjanggun/pull/73)이다. superseded PR과 clean worktree 정리는 완료됐다. 후속 보정에서는 일반 authenticated의 Preview token·draft showroom 접근, 공식 자산 공개 RLS의 정확한 asset/media 연결, Production `/test-fixtures/**` 중앙 404 경계를 수정했다. 다음 승인 단계는 보정 diff의 Draft 재검수 후 Ready 여부 판단이다. Production 배포·공개 글 발행·미디어 공개 승격은 이번 완료 사실에 포함하지 않는다. 어드민·블로그 최종 검증기는 state-scope invariant를 유지하며, 최종 실행 전에는 통과 개수를 임의로 적지 않는다.
