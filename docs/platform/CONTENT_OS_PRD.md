...
목표는 단순 블로그 게시판이 아니다. Codex가 중앙 문장군 브랜드 자료와 이 프로젝트의 CMS 데이터·발행 이력을 조사해 외부에서 작성한 원고를, 관리자가 사실관계, 금지표현, 사진, SEO/AEO 필드를 검수한 뒤 문장군 공식 콘텐츠로 발행하는 구조를 만든다. 별도 블로그 운영 저장소는 이 CMS의 데이터 원본이나 동기화 대상이 아니다.
...
| 사용자 | 역할 |
|---|---|
| Codex | 문장군_브랜드의 브랜드 사실·금지표현·현장 판단과 이 프로젝트 CMS의 기존 글·카테고리·중복·발행 상태를 조사해 원고와 SEO/AEO 필드를 외부 작성 |
...
1. Codex는 문장군_브랜드와 이 프로젝트의 CMS 데이터를 조사한 뒤, 본문 블록과 `summary_answer`, `target_question`, `primary_keyword`, `related_questions`를 포함한 원고를 외부에서 작성한다.
