        ...
        <header className={`${styles.hero} ${cover ? '' : styles.heroNoMedia}`}>
          <div className={styles.heroText}>
            <div className={styles.metaRow}>
              ...
              {updatedDate && <time dateTime={presentation.modifiedAt}>업데이트 {updatedDate}</time>}
            </div>
            <h1>{presentation.headline}</h1>
            {post.targetQuestion && <p className={styles.question}>{post.targetQuestion}</p>}
            {presentation.summaryAnswer && <p className={styles.summary}>{presentation.summaryAnswer}</p>}
