...
export default function VisitDatePicker({ selectedDate, onChange }: Props) {
  ...
  const checkDateAvailable = useMemo(() => {
    return (date: Date): { available: boolean; reason?: string } => {
      ...
      if (settings.close_saturday && dow === 6) return { available: false, reason: '토요일은 신청할 수 없습니다.' }
      if (settings.close_sunday && dow === 0) return { available: false, reason: '일요일은 신청할 수 없습니다.' }

      return { available: true }
    }
  }, [settings, overrides, todayStr])
  ...
  return (
    <div className={styles.pickerContainer}>
      ...
      <p className={styles.timeNotice}>
        정확한 방문 시간은 지역 동선과 당일 일정에 따라 배정되며, 방문 전날 오후 담당 매니저가 예상 시간을 안내드립니다.
      </p>
    </div>
  )
}
