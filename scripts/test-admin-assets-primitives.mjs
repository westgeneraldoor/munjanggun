...
assert.match(client, /const selectionAnchorId = selectionAnchorRef\.current[\s\S]*if \(range && selectionAnchorId\)[\s\S]*selectionAnchorId/, 'range selection must capture its anchor before React schedules the selected-set update')
...
assert.match(client, /draggable=\{false\}/, 'asset thumbnails must prevent native image drag from taking over range selection')
assert.match(client, /function handleCardKeyDown[\s\S]*event\.shiftKey[\s\S]*(?:Enter| )/, 'Shift+Enter and Shift+Space must preserve the range modifier through keyboard selection')
assert.match(client, /function handleCardCheckboxClick[\s\S]*event\.shiftKey/, 'checkbox clicks must preserve Shift before the native change event drops modifier state')
assert.match(client, /onClick=\{event => handleCardCheckboxClick\(item\.id, event\)\}[\s\S]*onChange=\{\(\) => handleCardCheckboxChange\(item\.id\)\}/, 'checkbox range selection must consume the modifier recorded by its click event')
assert.match(client, /dragRect\s*\?\s*\(\s*<div[\s\S]*?data-selection-drag-rect/, 'active pointer drag must render a visible selection rectangle')
assert.match(styles, /\.dragSelectionRect\s*\{[\s\S]*position:\s*fixed/, 'the selection rectangle must track viewport pointer coordinates')
...
assert.match(actions, /const CONTENT_ASSET_ID_PATTERN = \/\^\[0-9a-f\]\{8\}/, 'archive actions must define a standard UUID content-asset identifier pattern before invoking the protected RPC')
assert.match(actions, /assetIds\.filter\(value => CONTENT_ASSET_ID_PATTERN\.test\(value\)\)/, 'archive actions must use the standard UUID pattern for selected content-asset identifiers')
assert.doesNotMatch(actions, /\[0-9a-f-\]\{35\}/, 'archive actions must not reject ordinary 36-character UUID content-asset identifiers')
assert.match(client, /if \(changedIds\.size > 0\) router\.refresh\(\)/, 'a blocked archive result must render its usage notice without waiting for an unnecessary route refresh')
assert.match(client, /const \[isPending, setIsPending\] = useState\(false\)[\s\S]*async function submit\(\)[\s\S]*setResult\(next\)/, 'archive confirmation must commit its result even when no route refresh is needed')
