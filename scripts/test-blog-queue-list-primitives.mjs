...
assert.match(queue, /<PlatformTable[\s\S]*?<IntentPrefetchLink[\s\S]*?에디터 열기/)
...
assert.match(queueCss, /@media \(min-width: 980px\)\s*\{\s*\.mobileList\[data-platform-list\]\s*\{\s*display:\s*none/s, 'desktop mobile-list hiding must outrank shared PlatformList display rules regardless of CSS chunk order')
...
assert.match(queue, /\{ key: 'archived', label: '휴지통' \}/)
assert.match(queue, /\bPlatformModal\b/)
assert.match(queue, /\bpermanentlyDeleteBlogPost\b/)
assert.match(queue, /deleteConfirmation/)
assert.match(queue, /data-modal-initial-focus/)
assert.match(queue, /role=\{actionMessage\.ok \? 'status' : 'alert'\}/)
assert.match(queue, />삭제<\/button>/)
assert.match(queue, />복원<\/button>/)
assert.match(queue, />영구삭제<\/button>/)
assert.doesNotMatch(queue, /window\.confirm/)
assert.doesNotMatch(queue, /window\.location\.reload/)
assert.doesNotMatch(queue, /\{ key: 'archived', label: '보관' \}/)
assert.equal(
  [...queue.matchAll(/<IntentPrefetchLink[\s\S]*?href=\{`\/admin\/platform\/blog\/\$\{row\.id\}(?:\/preview)?`\}/g)].length,
  6,
  'every row title, edit, and preview link must use bounded hover/focus intent prefetch',
)
