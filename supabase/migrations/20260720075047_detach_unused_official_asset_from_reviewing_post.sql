-- Detach one unused official asset bridge from an unpublished reviewing post.
-- The content asset, its files, and the media audit row are deliberately kept.
...
BEGIN
  ...
  IF v_media_count <> 1 OR v_media_id IS NULL THEN
...
COMMENT ON FUNCTION showroom.detach_unused_official_asset_from_reviewing_post(UUID, UUID, UUID, TEXT, TEXT)
IS 'Service-role-only audited unlink for one unused private official asset bridge on a reviewing post.';
